...
async function getTemp(city){
    const response = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=8773e0117dbb4106ac0154641241201&q=${city}&days=3&aqi=no&alerts=no`)
    if(response.ok==true){
...
function getToday(){
    ...
    todayDate.innerHTML= todayDay.toLocaleDateString("en-US",{dateStyle:"full"})
...
function getAfterTom(){
    ...
    afterTomDate.innerHTML = afterTomDay.toLocaleDateString("en-US",{dateStyle:"full"})
    afterTomIcon.setAttribute('src',`https:${data.forecast.forecastday[2].day.condition.icon}`)
